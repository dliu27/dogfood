...
@schedule("* * * * *", job=simple_config_job)
def exception_schedule(context: ScheduleEvaluationContext):
...
@schedule("* * * * *", job=simple_config_job)
def skip_schedule(context: ScheduleEvaluationContext):
    tick_time = context.scheduled_execution_time  # Access the tick time
    if tick_time is not None:  # This should always be available for schedules
        tick_minute = tick_time.minute
        if tick_minute % 2 == 1:
...
@schedule("* * * * *", job=simple_config_job)
def timeout_schedule(context: ScheduleEvaluationContext):
    tick_time = context.scheduled_execution_time  # Access the tick time
    if tick_time is not None:  # This should always be available for schedules
        tick_minute = tick_time.minute
        if tick_minute % 2 == 1:
